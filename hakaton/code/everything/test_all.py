...
def align_to_objective(robot, initial_dist, initial_yaw_deg, target_distance=0.2):
    ...
    while rclpy.ok():
        ...
        if distance_error < robot.linear_threshold and abs(heading_error) < robot.angular_threshold:
            break

        angular_speed = clamp(robot.kp_angular * heading_error,
                              -robot.max_angular_speed, robot.max_angular_speed)
        linear_speed = robot.clamp_linear_speed(robot.kp_linear * distance_error,
                                                0.0, robot.max_linear_speed)
...
class PickupStateMachine:
    ...
    def _pos_adjust_not_final(self):
        """Подъезжает на 40 см от объекта."""
        self.robot.node.get_logger().info("State: pos_adjust_not_final (drive to 0.40m)")
        success = align_to_objective(self.robot, self.dist, -self.angle, target_distance=0.60)
        if success:
            self.state = "detect"
        else:
            self.state = "rotate"
    ...
    def _detect(self):
        ...
        if 0.35 < round(self.dist, 2) < 45 and self.angle > 22:
            print("We've spotted floor")
            # with open(f'image_36cm_{time.time()}.npy', 'wb') as f:
                # np.save(f, detector.latest_image)
            self.robot.drive(0.2, 0.0, 0.0)
            return False

        self.robot.node.get_logger().info(f"Measured: dist={self.dist:.2f}m, angle={self.angle:.1f}deg")
        print(f"{self.dist=}")
        if self.dist > 0.65:
            self.state = "pos_adjust_not_final"
        else:
    ...
    def _pos_adjust(self):
        ...
        success = align_to_objective(self.robot, self.dist, 0, target_distance=0.25)
        if success:
            self.state = "grab"
        else:
            self.state = "rotate"
    ...
    def _grab(self):
        ...
        # 3. Захват
        # self.robot.close_gripper(50.0)        
        time.sleep(10.0)
        # 4. Поднимаем руку в транспортное положение
        self.robot.move_arm_to(*ARM_PICK_MIDDLE_POS_AFTER_CHECK, 1)
        time.sleep(3.0)
        self.state = "verify"
        return False
